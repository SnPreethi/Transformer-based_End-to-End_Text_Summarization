...
class ModelEvaluation:
    ...
    def generate_batch_sized_chunks(self, list_of_elements, batch_size):
        '''Split the dataset into smaller batched that we can process simultaneously
        Yield successful batch-sized chunks from list_of_elements'''
    ...
    def calculate_metric_on_test_ds(self, dataset, metric, model, tokenizer, batch_size=16, device='cuda' if torch.cuda.is_available() else 'cpu', column_text='article', column_summary='highlights'):
        ...
        for article_batch, target_batch in tqdm(
            ...
            '''parameter for length penalty ensures that the model does not generate sequences that are too loo long'''
